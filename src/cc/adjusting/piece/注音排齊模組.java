...
class 注音排齊模組 extends 注音排放模組
{
	...
	Rectangle2D 上尾範圍()
	{
		double 活字大細 = Math.max(上尾活字.字範圍().getWidth(), 上尾活字
				.字範圍().getHeight());
		Rectangle2D.Double 格仔 = new Rectangle2D.Double(上尾活字.字範圍()
				.getMinX() - 活字大細 / 2.0, 上尾活字.字範圍().getMinY() - 活字大細
				/ 2.0, 活字大細, 活字大細);
		return 格仔;
	}
	...
	Rectangle2D 對齊範圍()
	{
		double 活字大細 = Math.max(對齊活字.字範圍().getWidth(), 對齊活字
				.字範圍().getHeight());
		Rectangle2D.Double 格仔 = new Rectangle2D.Double(對齊活字.字範圍()
				.getMinX() - 活字大細 / 2.0, 對齊活字.字範圍().getMinY() - 活字大細
				/ 2.0, 活字大細, 活字大細);
		return 格仔;
	}
}
