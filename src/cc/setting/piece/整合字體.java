...
public class 整合字體 implements 字體介面
{
	...
	public 整合字體 調整字體參數(float 字型大小)
	{
		整合字體 調整結果 = new 整合字體();
		調整結果.合併字體集 = new 合併字體[this.合併字體集.length];
		for (int i = 0; i < 合併字體集.length; ++i)
		{
			調整結果.合併字體集[i] = this.合併字體集[i].調整字體參數(字型大小);
		}
		return 調整結果;
	}

	public 整合字體 調整字體參數(int 字體選項)
	{
		整合字體 調整結果 = new 整合字體();
		調整結果.合併字體集 = new 合併字體[this.合併字體集.length];
		for (int i = 0; i < 合併字體集.length; ++i)
		{
			調整結果.合併字體集[i] = this.合併字體集[i].調整字體參數(字體選項);
		}
		return 調整結果;
	}

	public 整合字體 調整字體參數(int 字體選項, float 字型大小)
	{
		整合字體 調整結果 = new 整合字體();
		調整結果.合併字體集 = new 合併字體[this.合併字體集.length];
		for (int i = 0; i < 合併字體集.length; ++i)
		{
			調整結果.合併字體集[i] = this.合併字體集[i].調整字體參數(字體選項, 字型大小);
		}
		return 調整結果;
	}
